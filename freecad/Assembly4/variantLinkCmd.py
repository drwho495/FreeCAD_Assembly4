...
from freecad.Assembly4.Asm4_objects import VariantLink, ViewProviderVariant
...
class makeVariantLink:
    ...
    def GetResources(self):
        ...
        return {
            "MenuText": "Create a variant Part",
            "ToolTip": tooltip,
    ...
    def Activated(self):
        ...
        if Asm4.getSelectedVarLink():
            ...
            if (
                parent.TypeId == "App::Part"
                and parent.getParentGeoFeatureGroup() is None
            ):
    ...
    def onCreateLink(self):
        ...
        # only create link if there is a Part object and a name
            ...
            if (
                ...
                or App.ActiveDocument == selectedPart.Document
            ):
                # create the variant link with the user-provided name
                # variantLink = self.rootAssembly.newObject( 'App::Link', linkName )
                variantLink = App.ActiveDocument.addObject(
                    "Part::FeaturePython", linkName, VariantLink(), None, True
                )
