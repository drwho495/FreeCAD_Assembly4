...
class placePartUI:
    ...
    def __init__(self):
        ...
        try:
            ...
            assert (
                self.selectedObj.getTypeIdOfProperty("Placement")
        ...
        if self.old_EE and self.old_Parent:
            (old_Parent, old_parentPart, old_parentLCS) = self.splitExpression(
        ...
        if not lcs_found:
            lcs_found = self.attLCSlist.findItems(
                "(" + old_parentLCS + ")", QtCore.Qt.MatchContains
            )
        if lcs_found:
            # ... and select it
    ...
    def onParentList(self):
        ...
        if self.parentList.currentText() == "Parent Assembly":
            ...
            self.parentDoc.setText(
                parentPart.Document.Name + "#" + Asm4.labelName(parentPart)
            )
    ...
    def onDatumClicked(self):
        ...
        if self.attLCSlist.selectedItems():
            ...
            if a_Part == "Parent Assembly":
                ...
                linkDot = a_Part + "."
            Gui.Selection.addSelection(
                self.activeDoc.Name, self.rootAssembly.Name, linkDot + a_LCS + "."
            )
    ...
    def drawUI(self):
        ...
        self.mainLayout.addWidget(
            QtGui.QLabel("Select attachment LCS in parent Part :")
        )
