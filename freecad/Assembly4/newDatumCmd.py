...
class newDatum:
    ...
    def Activated(self):
        ...
        # something went wrong
            ...
            Asm4.warningBox(
                "Can't create a " + self.datumType + " with the current selections"
            )
        ...
        if parentContainer:
            ...
            if ok and text:
                ...
                if self.datumColor:
                    Gui.ActiveDocument.getObject(createdDatum.Name).ShapeColor = (
                        self.datumColor
                    )
                if self.datumAlpha:
                    Gui.ActiveDocument.getObject(createdDatum.Name).Transparency = (
                        self.datumAlpha
                    )
    ...
    def Activated(self):
        ...
        for i in range(len(edges)):
            ...
            if parentPart:
                ...
                if parentPart and (
                    ...
                    axis = Asm4.newLCS(
                        parentPart,
                        "PartDesign::Line",
                        "HoleAxis_" + str(instanceNum),
                        [(selectedObj, (edgeName,))],
                    )
            ...
            #
                ...
                FCC.PrintMessage(
                    "Datum objects can only be created inside Part or Body containers"
                )
...
Gui.addCommand(
    "Asm4_createDatum", Asm4.dropDownCmd(createDatumList, "Create Datum Object")
)
