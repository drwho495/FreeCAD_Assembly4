...
class placeLinkUI:
    ...
    def __init__(self):
        ...
        if Asm4.isAsm4EE(self.selectedObj):
            ...
            FCC.PrintWarning(
                "WARNING : unsupported Assembly/Solver/Part combination, you shouldn't be seeing this\n"
            )
        ...
        (self.old_Parent, separator, self.old_parentLCS) = (
            self.selectedObj.AttachedTo.partition("#")
        )
        (old_Parent, old_attLCS, old_linkLCS) = self.splitExpressionLink(
        ...
        # ... or with (Name)
        if not lcs_found:
            lcs_found = self.partLCSlist.findItems(
                "(" + old_linkLCS + ")", QtCore.Qt.MatchEndsWith
            )
        if lcs_found:
            # ... and select it
        ...
        if not lcs_found:
            lcs_found = self.attLCSlist.findItems(
                "(" + old_attLCS + ")", QtCore.Qt.MatchEndsWith
            )
        if lcs_found:
            # ... and select it
    ...
    def finish(self):
        ...
        Gui.Selection.addSelection(
            self.activeDoc.Name, self.rootAssembly.Name, self.selectedObj.Name + "."
        )
    ...
    def onParentSelected(self):
        ...
        # if something is selected
            ...
            if parentPart:
                ...
                Gui.Selection.addSelection(
                    parentPart.Document.Name,
                    self.rootAssembly.Name,
                    parentPart.Name + ".",
                )
                QtCore.QTimer.singleShot(
                    1500,
                    lambda: Gui.Selection.removeSelection(
                        parentPart.Document.Name,
                        self.rootAssembly.Name,
                        parentPart.Name + ".",
                    ),
    ...
    def addSelection(self, doc, obj, sub, pnt):
        ...
        if Asm4.isValidDatum(selObj) and len(selPath) > 2: #todo: fix
            ...
            if selLink == self.selectedObj:
                ...
                found = self.partLCSlist.findItems(
                    Asm4.labelName(selObj), QtCore.Qt.MatchExactly
                )
                if len(found) > 0:
            ...
            # if it's a child in the assembly:
                ...
                # find the parent
                idx = self.parentList.findText(
                    Asm4.labelName(selLink), QtCore.Qt.MatchExactly
                )
                if idx >= 0:
    ...
    def reorientLink(self):
        moveXYZ = App.Placement(
            App.Vector(self.Xtranslation, self.Ytranslation, self.Ztranslation),
            self.old_LinkRotation,
        )
    ...
    def splitExpressionLink(self, expr, parent):
        ...
        if nbHash == 0:
            ...
            if parent == "Parent Assembly":
                ...
                (attLCS, separator, rest1) = expr.partition(
                ...
                (attLCS, separator, rest2) = rest1.partition(
                    ".Placement * AttachmentOffset * "
                )
            ...
            if parent == "Parent Assembly":
                ...
                if (
                    parentObj
                    and parentObj.isDerivedFrom("App::Link")
                    and not parentObj.LinkedObject.Document
                    == self.rootAssembly.Document
                ):
    ...
    def drawUI(self):
        ...
        self.parentList.setToolTip(
            "Choose the part in which the attachment\ncoordinate system is to be found"
        )
        ...
        numberOfDecimals = App.ParamGet(
            "User parameter:BaseApp/Preferences/Units"
        ).GetInt("Decimals")

        # X Translation Value
        self.XoffsetLayout = QtGui.QHBoxLayout()
        self.XtranslSpinBoxLabel = self.XoffsetLayout.addWidget(
            QtGui.QLabel("X Translation :")
        )
        ...
        self.YtranslSpinBoxLabel = self.YoffsetLayout.addWidget(
            QtGui.QLabel("Y Translation :")
        )
        ...
        self.ZtranslSpinBoxLabel = self.ZoffsetLayout.addWidget(
            QtGui.QLabel("Z Translation :")
        )
